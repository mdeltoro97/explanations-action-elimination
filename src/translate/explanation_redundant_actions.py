...
def generating_explanations(plan, list_pos_redundant_actions, list_causal_links_sas_plan, list_causal_links_ae_sas_plan, task, causal_chain_list):
                            ...
                            for element in producers_list:
                                ...
                                if producer == 0:
                                    ...
                                    if redundant_action is None:
                                        explanation_str += f"\n--> {fact} as a precondition which is obtained through the effects produced by the relevant action {producer}."
                                    else:
...
def main():
    ...
    if (is_perfectly_justified(ae_plan)):
        ...
        generating_explanations(plan, list_pos_redundant_actions, list_cl_plan, list_cl_ae_plan, task, causal_chain_list)
